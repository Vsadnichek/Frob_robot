# frob_robot city navigation

Робот для автономной навигации по городу в рамках хакатона «Кубок РТК Высшая лига». Робот движется по полигону 4×4 м (сетка 5×5 ячеек 800×800 мм),
...
узла графа (конвенция: **0 = север**). Если параметр `initial_heading != 0`,
используется переданное значение (также в конвенции 0 = север).
...
Action сервер, принимающий список команд движения и выполняющий их последовательно
...
Координаты узлов графа хранятся в мировой системе координат (совпадает с полигоном SDF),
но одометрия DiffDrive-плагина публикуется в фрейме `odom` (робот стартует с 0,0).
Визуализатор переводит мировые координаты в одометрические, зная позицию `start_node`,
чтобы маркеры и робот совпадали.
...
| `start_node` | `-1` (авто) | Узел спавна робота (задаёт world→odom origin) |
...
- `suggested_heading` — рекомендуемый курс в радианах (**0 = север, π/2 = восток**)
...
При запуске симуляции камера автоматически устанавливается в top-down вид над центром
полигона через `--gui-config` → `config/gazebo_camera.config`:
позиция `(2, 2, 3)`, pitch=π/2, yaw=π/2.
